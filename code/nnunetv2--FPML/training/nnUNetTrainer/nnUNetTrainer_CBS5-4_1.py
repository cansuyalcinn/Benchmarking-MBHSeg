...
# This model is nnunet + ML + FP (2 FP images + original prediction) in each decoder only using main segmentation task. 
# we changed kap value to 0.1. we will try on seed 4 0.05.
# There is a contribution of the batch statistics module. 

class FeaturePerturbation(nn.Module):
    def __init__(self, lam=0.9, kap=0.1, eps=1e-6, use_gpu=True):
    ...
    def forward(self, x):
        ...
        lam = torch.tensor(self.lam, device=sig.device, dtype=sig.dtype)
        epsilon_tensor = torch.tensor(epsilon, device=sig.device, dtype=sig.dtype)

        gamma = lam * sig + (1 - lam) * batch_sig + epsilon_tensor * batch_phi
        beta = lam * mu + (1 - lam) * batch_mu + epsilon_tensor * batch_psi
...
class DualDecoderWrapper(nn.Module):
    ...
    def __init__(self, encoder, decoder, n_classes, num_FP=2):
        super().__init__()
...
class nnUNetTrainerCoBoundarySeg5_4_1(nnUNetTrainerNoDeepSupervision):
    """
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        y_all_fp = torch.zeros((num_outputs, num_FP) + outputs_seg[0].shape).to(self.device)
        for dec_idx in range(num_outputs):  # 0 and 1
            for fp_idx in range(num_FP):    # 0 and 1
...
class nnUNetTrainerCoBoundarySeg_100epochs5(nnUNetTrainerCoBoundarySeg5_4_1):
    def __init__(
            self,
            plans: dict,
            configuration: str,
