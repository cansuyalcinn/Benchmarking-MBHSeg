...
# 5-2_1: 
# This model is nnunet + ML. No FP. Border segmentation head and border loss is used.
# in each decoder using border and main segmentation head.
# Border loss is entegrated in semi supervised fashion. we calculate it on labeled and unlabeled data.
# Supervised losses are the main segmentation and border segmentation losses on labeled data.
# Unsupervosed losses are mutual learning losses of main segmentation and border segmentation heads.
...
class nnUNetTrainerCoBoundarySeg5_2_1(nnUNetTrainerNoDeepSupervision):
    """
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        # Border segmentation loss on the labeled data. (supervised loss)
        loss_border_labeled = 0
        for idx in range(len(outputs_border)):
            loss_border_labeled += self.loss(outputs_border[idx][:self.labeled_bs], target[:self.labeled_bs])
        ...
        # For the unlabeled border heads, calculate mutual learning loss.
        y_all_border = torch.zeros((len(outputs_border),) + outputs_border[0].shape).to(self.device)
        for idx in range(len(outputs_border)):
            y = outputs_border[idx]
            y_prob = F.softmax(y, dim=1)
            y_all_border[idx] = y_prob

        loss_ml_border = 0
        for i in range(len(outputs_border)):
            for j in range(len(outputs_border)):
                if i != j:
                    loss_ml_border += self.calculate_ML(y_all_border,
                                                        outfeats,
                                                        outputs_border,
                                                        i,
                                                        j,
        ...
        weighted_loss_ml = cw_ml  * loss_ml 
        self.logger.log('loss_ml', loss_ml.detach().cpu().numpy(), self.current_epoch)
        self.logger.log('consistency_weight', cw_ml, self.current_epoch)
        self.logger.log('weighted_loss_ml', weighted_loss_ml.detach().cpu().numpy(), self.current_epoch)

        sup_loss = 0.5 * (loss_seg + loss_border_labeled)
        unsup_loss = cw_ml * (loss_ml + loss_ml_border)

        l = sup_loss + unsup_loss
...
class nnUNetTrainerCoBoundarySeg_100epochs5(nnUNetTrainerCoBoundarySeg5_2_1):
    def __init__(
            self,
            plans: dict,
            configuration: str,
