...
# 5-4_8_2: 
# This model is nnunet + ML + FP (3 FP image + original prediction) + border loss.
# kap=0.1, # of FP=3, supervised border loss weight = 0.1 (same as ML loss weight) maximum reaching 0.1.
...
class nnUNetTrainerCoBoundarySeg5_4_8_2(nnUNetTrainerNoDeepSupervision):
    """
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        with torch.no_grad():
            border_targets = []
            for b in range(self.labeled_bs):
                # target[b] has shape [1, D, H, W]
                t_np = target[b, 0].cpu().numpy()
                border = get_3d_border(t_np, iterations=1)   # get GT border
                border_targets.append(torch.from_numpy(border).to(self.device))

            border_targets = torch.stack(border_targets, dim=0)   # [B, D, H, W]
            border_targets = border_targets.unsqueeze(1).float()  # [B,1,D,H,W]

        loss_border_labeled = 0
        for idx in range(len(outputs_border)):
            loss_border_labeled += self.loss(outputs_border[idx][:self.labeled_bs], border_targets)
        ...
        l = (0.5 * loss_seg) + weighted_loss_ml_fp + (cw_ml * loss_border_labeled)
...
class nnUNetTrainerCoBoundarySeg_100epochs5(nnUNetTrainerCoBoundarySeg5_4_8_2):
    def __init__(
            self,
            plans: dict,
            configuration: str,
