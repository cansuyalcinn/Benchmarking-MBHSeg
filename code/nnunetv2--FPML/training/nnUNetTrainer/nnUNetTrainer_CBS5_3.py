...
# This model is with nnunet + ML + Lborder_loss supervised only.  (0.25 weight)
...
def get_3d_border(mask_3d):
    ...
    """
    # print("shape mask_3d:", mask_3d.shape) # 96x96x96
    eroded = binary_erosion(mask_3d, iterations=3)
    border = mask_3d.astype(np.uint8) - eroded.astype(np.uint8)
    return border

class nnUNetTrainerCoBoundarySeg5_3(nnUNetTrainerNoDeepSupervision):
    """
    ...
    def __init__(
        ...
        print(f"Percentage of labeled data: {self.percentage_labeled_data}, Seed name: {self.seed_name}")
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        # # print(f"Data shape: {data.shape}, Target shape: {target.shape}") 4,1,96,96,96 and # 4,1,96,96,96
        border_labels = []
        for i in range(target.shape[0]):
            # print(f"Processing target {i} with shape {target[i].shape}") # 1x96x96x96
            border_label = get_3d_border(target[i].cpu().numpy().squeeze(0))
            # print(f"Border label shape: {border_label.shape}") # 96x96x96
            border_labels.append(border_label)

        border_labels = np.array(border_labels)
        border_labels = np.expand_dims(border_labels, axis=1) 
        border_labels = torch.from_numpy(border_labels).cuda()

        # print("shape border_labels:", border_labels.shape) # 4,1,96,96,96
        ...
        # Border loss on labeled data. 
        loss_dice_border = 0
        for idx in range(num_outputs):
            loss_dice_border += self.loss(outputs_border[idx][:self.labeled_bs], border_labels[:self.labeled_bs])
        ...
        l = ( 0.5 * loss_seg) + (cw_ml * loss_ml) + (loss_dice_border * 0.25)
...
class nnUNetTrainerCoBoundarySeg_100epochs5(nnUNetTrainerCoBoundarySeg5_3):
    def __init__(
            self,
            plans: dict,
            configuration: str,
