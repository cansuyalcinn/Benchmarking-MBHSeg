...
# 5-4_8_3: 
# This model is nnunet + ML + FP (3 FP image + original prediction) + uncertainty-aware mutual learning.
# kap=0.1, # of FP=3, and λ=0.2 the percentage of uncertain voxels=10%.
...
class nnUNetTrainerCoBoundarySeg5_4_8_3(nnUNetTrainerNoDeepSupervision):
    """
    ...
    def calculate_ML_FP_UA(self, y_all, outfeats, outputs_seg, y_all_fp, i, j, num_classes, consistency_criterion, uncertain_ratio=0.10, λ=0.01):
        '''
        Calculates Uncertainity-aware Mutual Learning with Feature Perturbation

        y_all_confident_j → most confident pseudo-labels (used already in our ML-FP pipeline).
        y_uncertain_confident_j → subset of those, but restricted to top 10% uncertain voxels.

        If λ = 0 → uncertain voxels = same weight as others (no effect).

        If λ = 1.0 → uncertain voxels get 2 times the weight of others.

        If λ = 0.5 → uncertain voxels get 1.5 times the weight.

        If λ = 2.0 → uncertain voxels get 3 times the weight, etc.
        ...
        # === STEP 2: Build y_all_confident_j (voxel-wise most confident prediction) ===
        stacked_probs_j = torch.stack(probs_j_all, dim=0)  # shape: (3, B, C, D, H, W)
        stacked_probs_j = stacked_probs_j.permute(1, 0, 2, 3, 4, 5)  # (B, 3, C, D, H, W)
        gather_idx = min_entropy_indices_j.unsqueeze(1)  # (B, 1, D, H, W)
        selected_probs_j = torch.gather(
            stacked_probs_j,
            dim=1,
            index=gather_idx.unsqueeze(2).expand(-1, -1, num_classes, -1, -1, -1)
        )  # (B, 1, C, D, H, W)
        y_all_confident_j = selected_probs_j.squeeze(1)  # (B, C, D, H, W)

        # === STEP 3: Confidence-based mutual supervision mask ===
        min_entropy_j = torch.gather(entropies_j_stack, dim=0, index=min_entropy_indices_j.unsqueeze(0)).squeeze(0)
        mask = (min_entropy_i > min_entropy_j).float()  # decoder j is more confident → it teaches i

        # === STEP 3b: NEW - Uncertainty-aware mask (top X% most uncertain voxels) ===
        flat_entropy = min_entropy_j.view(min_entropy_j.shape[0], -1)  # (B, N)
        k = int(flat_entropy.shape[1] * uncertain_ratio)  # top X%
        thresholds = torch.kthvalue(flat_entropy, flat_entropy.shape[1] - k, dim=1)[0]
        uncertain_mask = (min_entropy_j >= thresholds.view(-1, 1, 1, 1)).float()  # (B, D, H, W)
        ...
        index = torch.argmax(y_all_confident_j, dim=1, keepdim=True)
        prototype_bank = torch.zeros(batch_f, num_classes, c_f).cuda()
        for ba in range(batch_f):
            for n_class in range(num_classes):
                mask_temp = (index[ba] == n_class).float()
        ...
        # === STEP 5b: NEW - Reweight with uncertainty mask ===
        # uncertain voxels get (1 + λ) weight
        uncertainty_weight = (1.0 + λ * uncertain_mask)
        weight_pixel_t = weight_pixel_t * uncertainty_weight
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        for i in range(num_outputs):
            for j in range(num_outputs):
                if i != j:
                    loss_ml_fp += self.calculate_ML_FP_UA(y_all=y_all,
                                                outfeats=outfeats,
        ...
        l = (0.5 * loss_seg) + weighted_loss_ml_fp 
...
class nnUNetTrainerCoBoundarySeg_100epochs5(nnUNetTrainerCoBoundarySeg5_4_8_3):
    def __init__(
            self,
            plans: dict,
            configuration: str,
