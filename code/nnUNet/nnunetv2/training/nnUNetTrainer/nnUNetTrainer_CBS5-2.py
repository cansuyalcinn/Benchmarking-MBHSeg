...
class nnUNetTrainerCoBoundarySeg5_2(nnUNetTrainerNoDeepSupervision):
    """
    ...
    def do_split(self):
        ...
        tr_labeled_keys = tr_keys[:num_labeled]
        tr_unlabeled_keys = tr_keys[num_labeled:]
        ...
        self.print_to_log_file(f"Number of labeled training samples: {len(tr_labeled_keys)}")
        self.print_to_log_file(f"Number of unlabeled training samples: {len(tr_unlabeled_keys)}")

        # Just return the identifiers
        return tr_labeled_keys, tr_unlabeled_keys, val_keys
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        cw_ml = self.get_current_consistency_weight(start_epoch=0, end_epoch=1000, max_weight=0.2)
        ...
        l = ( (1-cw_ml) * loss_seg) + (cw_ml * loss_ml) 
...
class nnUNetTrainerCoBoundarySeg_100epochs5(nnUNetTrainerCoBoundarySeg5_2):
    def __init__(
            self,
            plans: dict,
            configuration: str,
